...
for(i in examinees){
  ...
  write.csv(df, here("tests", "test_output", "walker_a", Sys.Date(), paste0(i, "_test_dat.csv")))
  pb$tick()
}
...
files <- fs::dir_ls(here("tests", "test_output", "walker_a", Sys.Date()))
...
write.csv(together, here("tests", "agreement_data", paste0(Sys.Date(), "_walker-a_shiny_vs_observed.csv")))

test_that("Walker A all responses", {
  print("Walker A; test of equivalent responses & ability estimates")
  expect_true(
